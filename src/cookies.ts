...
export default async (): Promise<string> => {
    ...
    if (
        ...
        if (response.encrypted) {
            ...
            if (data.cookie_data['douyu.com']) {
                logger.info('使用CookieCloud中的Cookies');

                const cookies = data.cookie_data['douyu.com']
                    .filter((cookie) => cookie.domain === 'douyu.com' || cookie.domain === '.douyu.com' || cookie.domain === 'www.douyu.com')
        ...
        logger.warn('CookieCloud中未找到douyu.com的Cookies');
    }
